...
2. Install feature and user packages with the native package manager.
3. Merge feature and user tools into the generated mise config and install them.
4. Install missing feature integrations without refreshing existing ones.
...
Available tiling variants are =aerospace=, =glazewm=, =komorebi=, =yabai=,
=niri=, and =mangowm=. Niri and MangoWM target Fedora Linux; MangoWM bootstraps
the official Terra repository, starts with the scrolling layout, and switches
between scrolling and dwindle with =Caps+Super+V=. Helix supports =standard=
(official release) and =steel=
(plugin-enabled fork). Changing a variant does not uninstall packages from the
