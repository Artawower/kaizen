...
Takes eval results (from trigger_evaluation.py) and generates an improved description
by calling `claude -p` as a subprocess (same auth pattern as trigger_evaluation.py —
...
def _call_claude(prompt: str, model: str | None, timeout: int = 300) -> str:
    ...
    # programmatic subprocess usage is safe. Same pattern as trigger_evaluation.py.
...
def main():
    ...
    parser.add_argument(
        "--eval-results",
        required=True,
        help="Path to eval results JSON (from trigger_evaluation.py)",
    )
