...
/// compatibility).  Each `[shortcuts]` entry is an array of tokens; any token that
/// matches a key in `[modifiers.darwin]` or `[modifiers.linux]` (chosen by `os`) is
/// replaced with its platform-native value.  Literal elements pass through unchanged.
///
