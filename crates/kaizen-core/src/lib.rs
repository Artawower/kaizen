...
impl KaizenEngine {
    ...
    /// 2. `manifest.toml` — authoritative list for non-Nix / fresh machines.
    /// 3. Scanning `features/*.toml` — legacy / UPT-only fallback.
    ...
    pub fn build_workflow_plan(
        ...
        // 2. manifest.toml — authoritative list for non-Nix / fresh machines.
        // 3. Scan features/*.toml — legacy fallback.
        let all_names: Vec<String> = if let Some(ref cache_path) = self.nix_cache_path {
            if let Some(map) = nix_feature_cache::load(cache_path, self.fs.as_ref())? {
                map.into_keys().collect()
            } else {
                self.names_from_manifest_or_store(&store)?
            }
        } else {
            self.names_from_manifest_or_store(&store)?
        };
    ...
    fn names_from_manifest_or_store(
        &self,
        store: &FeatureStore,
    ) -> Result<Vec<String>, KaizenError> {
        let m = self.load_manifest()?;
        if !m.features.is_empty() {
            return Ok(m.features.into_iter().map(|f| f.name).collect());
        }
