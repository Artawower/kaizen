...
// ── Unified feature+variants wizard (stubbed in steel-poc phase A) ───────────

/// Returns None (wizard not available). Placeholder until Steel VM replaces feature model.
pub fn pick_features_with_variants<F>(
    _prompt: &str,
    _initial_features: Vec<()>,
    _initial_experimental: bool,
    _feature_reloader: F,
) -> anyhow::Result<Option<(Vec<String>, std::collections::BTreeMap<String, String>)>>
where
    F: Fn(bool) -> anyhow::Result<Vec<()>>,
{
    Ok(None)
}
